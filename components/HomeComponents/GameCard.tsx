...
const GameCard: React.FC<GameCardProps> = ({
  ...
  */
  ...
  const getOtherProfileImage = async (user: string) => {
    try {
      console.log('ahem', match);
      console.log(user);
      console.log(match[user]);

      const otherUser = await axios.post(`${serverUrl}/getUser`, {
        userID: match[user].userID,
      });
      console.log('Ayy', otherUser);
      if (otherUser.data.hasDefaultProfileImage == 'true') {
        const urlDefault = otherUser.data.defaultProfileImage;

        console.log('other person has default', urlDefault);

        setOtherProfileUri(imageMap[urlDefault]);
      } else {
  ...
  const handleDismiss = () => {
    setActiveMatches(prevMatches => {
      ...
      console.log(
        'Matches after Dismiss',
        prevMatches.filter(match => match !== matchID),
      );
  ...
  const currentIndex = useDerivedValue(() => {
    if (yourFormattedData && match) {
      ...
      console.log(
        ((positionValue / (width - 60)) * yourFormattedData.length) / fraction,
      );
  ...
  useAnimatedReaction(
    () => currentIndex.value,
    () => {
      runOnJS(setAnimatedIndex)(currentIndex.value);
      ...
      {!loading && match && yourFormattedData ? (
        <>
          <View
            ...
            } /*style={styles.gameCardContainer}*/ /*onPress={() => {
                ...
                <View
                  ...
                  <View
                    ...
                    <Text style={[styles.percentText, {color: oppColor}]}>
                      {((oppTotalPrice - 100000) / (0.01 * 100000)).toFixed(2)}%
                    </Text>
                  </View>
                </View>
                <View
