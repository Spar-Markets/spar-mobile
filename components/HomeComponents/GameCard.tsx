...
import {
  ...
  deleteMatch,
  initializeMatch,
  updateOppTickerPrices,
  updateYourTickerPrices,
} from '../../GlobalDataManagment/matchesSlice';
...
const GameCard: React.FC<GameCardProps> = ({
  ...
  const [yourFormattedData, setYourFormattedData] = useState<any[] | null>(
    null,
  );
  ...
  const setUserMatchData = async (
    ...
    try {
      ...
      const yourSnapshots: PortfolioSnapshot[] =
        snapshots.data[`${yourUserNumber}Snapshots`];
      ...
      const oppSnapshots: PortfolioSnapshot[] =
        snapshots.data[`${opponentUserNumber}Snapshots`];
      ...
      console.log(
        'YOUR ASSETS FROM MATCH OBJECT:',
        match[yourUserNumber].assets,
      );
      ...
      dispatch(
        addOrUpdateMatch({
          matchID,
          yourAssets: match[yourUserNumber].assets,
          opponentAssets: match[opponentUserNumber].assets,
          yourBuyingPower: match[yourUserNumber].buyingPower,
          oppBuyingPower: match[opponentUserNumber].buyingPower,
        }),
      );
      ...
      const response = await axios.post(serverUrl + '/getUsernameByID', {
        userID: match[opponentUserNumber].userID,
      });
  ...
  const formatData = async () => {
    const sourceData = yourPointData.filter(
    ...
    const sourceData2 = opponentPointData.filter(
      (item: any, index: any) => index % 2 === 0,
    );
    ...
    const data2Max = Math.max(
      ...data2.map((item: any) => item.normalizedValue),
    );
    const dataMin = Math.min(...data.map((item: any) => item.normalizedValue));
    const data2Min = Math.min(
      ...data2.map((item: any) => item.normalizedValue),
    );
  ...
  const getInitialPrices = async () => {
    ...
    if (yourAssets && opponentAssets) {
      try {
        ...
        const yourInitialTickerPricesArray: any = await Promise.all(
          yourInitialTickerPromises,
        );
        const yourInitialTickerPrices = yourInitialTickerPricesArray.reduce(
          (acc: any, item: any) => {
        ...
        dispatch(
          updateYourTickerPrices({
            matchID: matchID,
            yourTickerPrices: yourInitialTickerPrices,
          }),
        );
        ...
        const oppInitialTickerPricesArray: any = await Promise.all(
          oppInitialTickerPromises,
        );
        const oppInitialTickerPrices = oppInitialTickerPricesArray.reduce(
          (acc: any, item: any) => {
            acc[item.ticker] = item.currentPrice;
            return acc;
          },
        ...
        dispatch(
          updateOppTickerPrices({
            matchID: matchID,
            oppTickerPrices: oppInitialTickerPrices,
          }),
        );
  ...
  function getNewTickerObject(jsonMessage: any, yourAssets: any) {
    ...
    const yourAssetsMap = new Map(
      yourAssets.map((asset: any) => [asset.ticker, asset]),
    );
    ...
    if (match && match.endAt) {
      ...
      if (timeUntilEnd <= 0) {
        ...
        const timeoutId = setTimeout(() => {
          ...
          setMatchIsOver(true);
          console.log(
            'CURRENT CLOSING THE GAMECARD WEBSOCKET SINCE MATCH ENDED',
          );
          ws.current!.close();
          ws.current = null;
          //dispatch(removeWebSocket(matchID))
  ...
  */
    ...
    ws.current.onmessage = async event => {
      ...
      try {
        ...
        if (JSONMessage.type == 'updatedAssets') {
          ...
          const yourNewAsset = getNewTickerObject(
            yourUpdatedAssets,
            yourAssets,
          );
          const oppNewAsset = getNewTickerObject(
            oppUpdatedAssets,
            opponentAssets,
          );
          ...
          for (const asset of yourUpdatedAssets) {
            if (!(asset.ticker in yourTickerPrices)) {
              newYourTickerPrices[asset.ticker] = await getCurrentPrice(
                asset.ticker,
              );
            }
          }
          ...
          for (const asset of oppUpdatedAssets) {
            if (!(asset.ticker in oppTickerPrices)) {
              newOppTickerPrices[asset.ticker] = await getCurrentPrice(
                asset.ticker,
              );
            }
          }
          ...
          const isTickerInYourAssets = yourAssets.some(
            stock => stock.ticker === sym,
          );
          const isTickerInOppAssets = opponentAssets.some(
            stock => stock.ticker === sym,
          );
          ...
          if (isTickerInYourAssets) {
            ...
            dispatch(
              updateYourTickerPrices({
                matchID,
                yourTickerPrices: updatedPrices,
              }),
            );
          ...
          if (isTickerInOppAssets) {
            ...
            dispatch(
              updateOppTickerPrices({matchID, oppTickerPrices: updatedPrices}),
            );
  ...
  const setupSocket = async () => {
    ...
    ws.current.onopen = () => {
      console.log(
        `Connected to GameCard Websocket, but not ready for messages...`,
      );
      if (ws.current! && yourAssets && opponentAssets) {
        //dispatch(addWebSocket({ id: matchID, ws: ws.current }));
        console.log(
          `Connection for GameCard Websocket is open and ready for messages`,
        );
        // first send match ID
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (yourFormattedData && yourFormattedData.length >= 1) {
        yourFormattedData[yourFormattedData.length - 1].normalizedValue =
          yourTotalLivePrice + yourBuyingPower - 100000;
      }
      ...
      if (oppFormattedData && oppFormattedData.length >= 1) {
        oppFormattedData[oppFormattedData.length - 1].normalizedValue =
          oppTotalLivePrice + oppBuyingPower - 100000;
      }
      /**/
      if (yourFormattedData && oppFormattedData) {
        //console.log(yourTotalLivePrice, oppTotalLivePrice)
        if (
          yourTotalLivePrice + yourBuyingPower >=
          oppTotalLivePrice + oppBuyingPower
        ) {
          setYourColor(theme.colors.stockUpAccent);
          setOppColor(theme.colors.tertiary);
        } else {
        ...
        const yourMax = Math.max(
          ...yourFormattedData.map((item: any) => item.normalizedValue),
        );
        const oppMax = Math.max(
          ...oppFormattedData.map((item: any) => item.normalizedValue),
        );
        const yourMin = Math.min(
          ...yourFormattedData.map((item: any) => item.normalizedValue),
        );
  ...
  const length = 10;
  const arrayWithZeroValues = Array.from({length}, (_, index) => ({
    index,
    value: 0,
  }));
  ...
  const [otherHasDefaultProfileImage, setOtherHasDefaultProfileImage] =
    useState(false);
    ...
    try {
      ...
      try {
        ...
        if (userInMongoResponse.status === 200) {
          ...
          setOtherHasDefaultProfileImage(
            userInMongoResponse.data.hasDefaultProfileImage,
          );
          const defaultProfileIndex = Number(
            userInMongoResponse.data.defaultProfileImage,
          );
  ...
  if (loading && initialDataLoad) {
    return <View style={{height: 250, width: width}}></View>;
  }
  ...
  useDerivedValue(() => {
    ...
    if (yourFormattedDataLength !== 0 && match) {
      const startTime =
        new Date(match.endAt).getTime() - match.timeframe * 1000;
      const fraction = (Date.now() - startTime) / (match.timeframe * 1000);
      const positionValue = state.x.position.value;
      const index = Math.round(
        (positionValue / (width - 40)) * (yourFormattedDataLength / fraction),
      );
      ...
      {matchIsOver && (
        <BlurView
          ...
          <View style={{flexDirection: 'row', gap: 5}}>
            <TouchableOpacity
              onPress={() => {
                setActiveMatchSummaryMatchID(matchID);
                expandMatchSummarySheet();
              }}
              style={{
      ...
      {!loading && match && yourFormattedData && oppFormattedData ? (
        <LinearGradient
          ...
          <ScrollView
            ...
            <View
              style={{
                /*backgroundColor: theme.colors.background*/ flexDirection:
                  'row',
                marginTop: 0,
                height: 180,
                gap: 10,
                ...
                {!isActive ? (
                  <>
                    ...
                    <View
                      ...
                      <Text style={[styles.percentText, {color: yourColor}]}>
                        {((yourTotalPrice - 100000) / (0.01 * 100000)).toFixed(
                          2,
                        )}
                        %
                      </Text>
                  ...
                  <>
                    <Text
                      ...
                      $
                      {yourFormattedData[animatedIndex]
                        ? yourFormattedData[animatedIndex].value.toFixed(2)
                        : yourTotalPrice.toFixed(2)}
                    </Text>
                    <View
                      style={[
                ...
                {!isActive ? (
                  <>
                    ...
                    <View
                      style={[
                        styles.percentIndicator,
                        {backgroundColor: hexToRGBA(oppColor, 0.3)},
                      ]}>
                      <Text style={[styles.percentText, {color: oppColor}]}>
                        {((oppTotalPrice - 100000) / (0.01 * 100000)).toFixed(
                          2,
                        )}
                        %
                      </Text>
                  ...
                  <>
                    <Text
                      ...
                      $
                      {oppFormattedData[animatedIndex]
                        ? oppFormattedData[animatedIndex].value.toFixed(2)
                        : oppTotalPrice.toFixed(2)}
                    </Text>
                    <View
                      style={[
