...
import { Image, StatusBar, StyleSheet, Text, TouchableOpacity, View, useColorScheme, NativeModules, ScrollView, FlatList } from 'react-native';
    ...
    <View style={[colorScheme == "dark" ? {backgroundColor: '#181818'} : {backgroundColor: '#fff'}, {flex: 1}]}>
        ...
        <View style={{flexDirection: 'row'}}>
                ...
                <View style={{alignItems: 'center', justifyContent: 'center', flex: 1}}>
                    <Text style={[colorScheme == "dark" ? {color: "#fff"} : {color: '#000'}, {marginHorizontal: 15, fontFamily: 'InterTight-Black', fontSize: 20}]}>Transfers</Text>
                </View>
                <View style={{flex: 1}}/>
            </View>
        </View>
        ...
        {transferData.map((item:any, index) => (
            ...
            <View key={index} style={{marginHorizontal: 12, marginVertical: 15, height: 50, flexDirection: 'row'}}>
                <View style={{flex: 1, justifyContent: 'center'}}>
                    <Text style={[colorScheme == "dark" ? {color: '#fff'} : {backgroundColor: '#000'}, {fontSize: 21, fontFamily: 'InterTight-Black'}]}>${item.amount}</Text>
                    <Text style={{color: '#888888', fontSize: 12, fontFamily: 'InterTight-Black'}}>{item.type}</Text>
                </View>
                <View style={{justifyContent: 'center'}}>
                    <Text style={[colorScheme == "dark" ? {color: '#fff'} : {backgroundColor: '#000'}, {fontSize: 12, fontFamily: 'InterTight-Black'}]}>{item.created.split("T")[0]}</Text>
                    <Text style={[colorScheme == "dark" ? {color: '#fff'} : {backgroundColor: '#000'}, {fontSize: 12, fontFamily: 'InterTight-Black'}]}>{item.status}</Text>
                </View>
            </View>
