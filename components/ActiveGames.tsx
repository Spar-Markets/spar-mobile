  ...
  return (
    <View style={{flex: 1}}>
      ...
      <ScrollView
          ...
          matchData.map((item, index) => (
            <View key={index} style={{flexDirection: 'row'}}>
              {item && 'wagerAmt' in item && 'user1' in item && (
                <GameCard
                  amountWagered={item!.wagerAmt}
                  mode={'Stock'}
                  idIndex={index}
                  yourPercentChange={3.54}
                  opp={item!.user1.userID}
                  oppPercentChange={1.56}
        ...
        {selectedMode == 'TOURNAMENTS' ? <View></View> : <View></View>}
      </ScrollView>
    </View>
  );
};
