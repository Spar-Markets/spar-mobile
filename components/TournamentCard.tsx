...
const TournamentCard = (props:any) => {
    ...
    const [timeRemaining, setTimeRemaining] = useState(calculateTimeRemaining(props.startDate))
    ...
    return (
        <TouchableOpacity style={{marginBottom: 10, marginLeft: 15, width: 300, maxWidth: screenWidth-30, borderRadius: 12, justifyContent: 'space-between'}}>
            <LinearGradient colors={['#272743', '#43436e']} start={{x: 0, y: 0}}
                end={{x: 1, y: 1}} style={{flex: 1, borderRadius: 12, borderWidth: 2, borderColor: '#3a3a61'}}>
                <View style={{flexDirection: 'row', justifyContent: 'center', alignItems: 'center', margin: 15}}>
                    <Text style={{fontFamily: 'InterTight-Bold', fontSize: 22, color: '#fff'}}>{props.name}</Text>
                    <View style={{flex: 1}}></View>
                    <View style={{borderRadius: 6, backgroundColor: '#1ae79c'}}>
                        <Text style={{padding: 5, fontFamily: 'InterTight-Bold', fontSize: 16, color: '#43436e'}}>${props.entryFee}</Text>
                    </View>
                </View>
                <View style={{flexDirection: 'row', width: '100%', height: 60, borderBottomLeftRadius: 12, borderBottomRightRadius: 12}}>
                <View style={{margin: 15}}>
                    <Text style={{fontFamily: 'InterTight-Bold', fontSize: 12, color: '#fff'}}>Starting in</Text>
                    <Text style={{fontFamily: 'InterTight-Bold', color: '#fff'}}>24 Minutes</Text>
                </View>
                <View style={{margin: 15}}>
                    <Text style={{fontFamily: 'InterTight-Bold', fontSize: 12, color: '#fff'}}>Prize Pool</Text>
                    <Text style={{fontFamily: 'InterTight-Bold', color: '#fff'}}>$1,040</Text>
                </View>
                <View style={{margin: 15}}>
                    <Text style={{fontFamily: 'InterTight-Bold', fontSize: 12, color: '#fff'}}>Players</Text>
                    <Text style={{fontFamily: 'InterTight-Bold', color: '#fff'}}>52</Text>
                </View>
            </View>
            </LinearGradient>
        </TouchableOpacity>
